...
My97DP.prototype = {
    /**
      ...
      */
	init: function() {
    if("yyyy年第M季度" == $dp.dateFmt){
    	$d.MI.disabled = true;
    	$d.MI.style.cursor="default";
        $lang.aMonStr=["","","","","","","","","","","",""];        
    }
	$ny = 0;
    $dp.cal = this;
    if ($dp.readOnly && $dp.el.readOnly != null) {
      $dp.el.readOnly = true;
      //去掉该行代码，解决当readOnly为true时tab键切换异常问题 赵美丹 2012-10-31
      //$dp.el.blur()
    }
    this._dealFmt();
     ...
     */
    if("yyyy年第M季度" == $dp.dateFmt){
    	hide($d.leftImg,$d.rightImg,$d.todayI);
    }
  },
  _makeDateInRange: function() {
    ...
    if (_ != 0) {
      ...
      //解决alwaysUseStartDate为true时快速选择范围不是以startDate为基准，切换年度时，快速选择范围不友好的问题 赵美丹 2013-04-19
  ...
  loadDate: function() {
    ...
    if (_ != "") {
      ...
      //解决设置startDate后，当startDate和最大最小值比较后不合法时会提示不合法的问题（修改为仅用输入框中的值做校验） 赵美丹 2014-04-19
  ...
  initShowAndHide: function() {
    ...
    ($dp.has.showM!=undefined ? $dp.has.showM : $dp.has.M) ? ($ = 1, show($d.MI, $d.leftImg, $d.rightImg)) : hide($d.MI, $d.leftImg, $d.rightImg);
  ...
  initQS: function() {
    ...
    else if("yyyy年第M季度" == $dp.dateFmt) for (H = 0; H < 7; H++) C[H] = $dt[_] - 3 + H;
...
function _tab(M) {
  ...
  if ($dp.enableKeyboard && isShow) {
    ...
    if (Q >= 37 && Q <= 40) {
      ...
      //修改代码，解决tab键切换仅在日历选择层切换，而非输入框之间的切换问题 赵美丹 2012-10-31
      if ($c.currFocus != D && $c.currFocus != $dp.el) {
        $c.currFocus = D;
        D.focus();
        return false;
      }
      //新增代码，实现tab键切换失去焦点时日历选择层自动关闭功能 赵美丹 2012-10-31
      if(H == $dp.el) {
    	  $c.close();
      }
    } else if (Q == 13) {
